...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances=1 \
  --cpu-throttling \
  --set-env-vars="AI_PROVIDER=gemini,AI_PROVIDER_TEXT=gemini,AI_PROVIDER_IMAGE=gemini,AI_PROVIDER_FILE=gemini,AI_PROVIDER_VOICE=gemini,AI_PROVIDER_VIDEO=gemini,AI_PROVIDER_ROUTING=gemini,MINIMAX_BASE_URL=https://api.minimax.io/v1,MINIMAX_MODEL=minimax-m2.7,MINIMAX_STT_MODEL=#g1_whisper-large" \
  $SECRETS_ARG \
  --project="$PROJECT_ID"
