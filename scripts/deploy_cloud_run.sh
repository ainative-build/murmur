...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances=1 \
  --cpu-throttling \
  --set-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=global,AI_PROVIDER=minimax,AI_PROVIDER_TEXT=minimax,AI_PROVIDER_IMAGE=minimax,AI_PROVIDER_FILE=minimax,MINIMAX_BASE_URL=https://api.minimax.io/v1,MINIMAX_MODEL=MiniMax-Text-01" \
  $SECRETS_ARG \
  --project="$PROJECT_ID"
